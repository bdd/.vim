" http://cvs.pld-linux.org/cgi-bin/viewvc.cgi/cvs/packages/vim-syntax-vcl/vcl.vim
"
" Vim syntax file
" Filename:     vcl.vim
...
syn match  vclOption   /now/
" client
syn match  vclOption   /client\.\(ip\|identity\)/
" server
syn match  vclOption   /server\.\(hostname\|identity\|ip\|port\)/
" req
syn match  vclOption   /req\.\(request\|url\|proto\|backend\|backend\.healthy\|hash_always_miss\|hash_ignore_busy\|can_zip\|restarts\|esi\|esi_level\|grace\|xid\)/
" bereq
syn match  vclOption   /bereq\.\(request\|url\|proto\|connect_timeout\|first_byte_timeout\|between_bytes_timeout\)/
" beresp
syn match  vclOption   /beresp\.\(do_stream\|do_esi\|do_gzip\|do_gunzip\|proto\|status\|response\|ttl\|grace\|saintmode\|backend\.name\|backend\.ip\|backend\.port\|backend\.storage\)/
" obj
syn match  vclOption   /obj\.\(proto\|status\|response\|ttl\|lastuse\|hits\|hash\|grace\)/
" resp
